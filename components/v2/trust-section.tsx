          ...
          {stats.map((stat, index) => (
            <div key={index} className="text-center">
              ...
              {stat.value ? (
                ...
                // Rivoir sets stat labels but no numeric values — render the label as the
                // prominent line so the tile still reads as a stat tile (no blank number).
                <div className="text-xl md:text-2xl font-bold text-[#0F1D2F] leading-snug">{stat.label}</div>
              )}
            </div>
          ))}
        </div>
